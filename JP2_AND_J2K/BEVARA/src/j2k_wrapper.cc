...
class FileHistogramInstance : public pp::Instance {
 ...
 private:
  ...
  void SaveRawImage(){
	 ...
	 //dictionary.Set(pp::Var("command"), pp::Var("debugme"));
	 //dictionary.Set(pp::Var("array"), ppm_header);
	 ...
	 memcpy(data+ ppm_header.length(), img, (imgWidth*imgHeight*3) );

	 // send to be downloaded
	 dictionary.Set(pp::Var("command"), pp::Var("j2kBlobReady"));
  ...
  void DecodeImage(pp::VarArrayBuffer& buffer) {
	...
	decodeJ2K(buffer_data, buffer_size);

	// dictionary.Set(pp::Var("command"), pp::Var("debugInfo"));
        //            dictionary.Set(pp::Var("dbgstring"), pp::Var(debugString));
        //            dictionary.Set(pp::Var("dbgval"), pp::Var(debugVal));
        //            PostMessage(dictionary);
