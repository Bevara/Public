...
fz_malloc_array_no_throw(fz_context *ctx, unsigned int count, unsigned int size)
{
	...
	if (count > UINT_MAX / size)
	{
		printf("error: malloc of array (%d x %d bytes) failed (integer overflow)", count, size);
		return NULL;
	}
...
fz_calloc_no_throw(fz_context *ctx, unsigned int count, unsigned int size)
{
	...
	if (count > UINT_MAX / size)
	{
		printf("error: calloc (%d x %d bytes) failed (integer overflow)\n", count, size);
		return NULL;
	}
...
fz_resize_array_no_throw(fz_context *ctx, void *p, unsigned int count, unsigned int size)
{
	...
	if (count > UINT_MAX / size)
	{
		printf( "error: resize array (%d x %d bytes) failed (integer overflow)\n", count, size);
		return NULL;
	}
...
/* 		printf("Lock %d not held when expected\n", lock); */
/* } */
...
/* 	if (fz_locks_debug[idx][lock] != 0) */
/* 		printf( "Lock %d held when not expected\n", lock); */
/* } */
...
/* 	if (fz_locks_debug[idx][lock] != 0) */
/* 	{ */
/* 		printf( "Attempt to take lock %d when held already!\n", lock); */
/* 	} */
/* 	for (i = lock-1; i >= 0; i--) */
/* 	{ */
/* 		if (fz_locks_debug[idx][i] != 0) */
/* 		{ */
/* 			printf("Lock ordering violation: Attempt to take lock %d when %d held already!\n", lock, i); */
/* 		} */
/* 	} */
...
/* 	if (fz_locks_debug[idx][lock] == 0) */
/* 	{ */
/* 		printf( "Attempt to release lock %d when not held!\n", lock); */
/* 	} */
