...
next_null(fz_stream *stm, int max)
{
  ...
  printf("in next NULL\n");
	...
	if (state->remain == 0)
		return EOF;
	/* printf("\t heading to fz_seek with buffer\n"); */
	fz_seek(state->chain, state->offset, 0);

	/* printf("\t heading to fz_available with buffer\n"); */
	/* printf("\t\t bytes avail should be %d - %d = %d\n",state->chain->wp,state->chain->rp,(state->chain->wp)-(state->chain->rp)); */
	n = fz_available(state->chain, max);

	/* printf("\t\t bytes avail are 0x%08x\n",n); */
	/* printf("\t checking state remain\n"); */
	if (n > state->remain)
		n = state->remain;
	...
	/* printf("\t should return %c\n",(char) *state->buffer); */
	n = *stm->rp;
	/* printf("\t\t returning the char %c\n", (char) n); */
	stm->rp++;
	return n;
	/* return *stm->rp++; */
}
