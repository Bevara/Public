...
/* { */
/* 	return (x < 0) ? myfloorf(x) : myceilf(x); */
/* } */
...
fz_paint_image_imp(fz_pixmap *dst, const fz_irect *scissor, fz_pixmap *shape, fz_pixmap *img, const fz_matrix *ctm, byte *color, int alpha, int lerp_allowed)
{
	...
	/* if (!is_rectilinear) */
	/* 	dolerp = lerp_allowed; */
	/* if (mysqrtf(local_ctm.a * local_ctm.a + local_ctm.b * local_ctm.b) > img->w) */
	/* 	dolerp = lerp_allowed; */
	/* if (mysqrtf(local_ctm.c * local_ctm.c + local_ctm.d * local_ctm.d) > img->h) */
	/* 	dolerp = lerp_allowed; */

	/* except when we shouldn't, at large magnifications */
	/* if (!img->interpolate) */
	/* { */
	/* 	if (mysqrtf(local_ctm.a * local_ctm.a + local_ctm.b * local_ctm.b) > img->w * 2) */
	/* 		dolerp = 0; */
	/* 	if (mysqrtf(local_ctm.c * local_ctm.c + local_ctm.d * local_ctm.d) > img->h * 2) */
	/* 		dolerp = 0; */
	/* } */
