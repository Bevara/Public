...
fz_subpixel_adjust(fz_matrix *ctm, fz_matrix *subpix_ctm, unsigned char *qe, unsigned char *qf)
{
	...
	pix_e = myfloorf(subpix_ctm->e);
	subpix_ctm->e -= pix_e;
	subpix_ctm->f = ctm->f + r;
	pix_f = myfloorf(subpix_ctm->f);
	subpix_ctm->f -= pix_f;
...
/* 		if (font->ft_face) */
/* 			return NULL; */
/* 		subpix_scissor.x0 = scissor->x0 - myfloorf(ctm->e); */
/* 		subpix_scissor.y0 = scissor->y0 - myfloorf(ctm->f); */
/* 		subpix_scissor.x1 = scissor->x1 - myfloorf(ctm->e); */
/* 		subpix_scissor.y1 = scissor->y1 - myfloorf(ctm->f); */
