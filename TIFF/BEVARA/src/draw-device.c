...
/* static float myfloorf(float x) { 
    int xi = (int)x;
    return x < xi ? (float) xi - 1 : (float) xi;
}

static float myceilf(float x) {
  int xi = (int)x;
  return x > xi ? (float) xi + 1 : (float) xi;
}
*/
...
/* 	printf("flatness = %f\n",flatness); */
/* 	printf("going to fz_device gray\n"); */
...
/* 	printf("going to rset gel\n"); */
/* 	fz_reset_gel(dev->gel, &state->scissor); */

/* 	printf("going to flatten fill\n"); */
/* 	fz_flatten_fill_path(dev->gel, path, ctm, flatness); */


/* 	printf("\t left flatten fill\n"); */


/* 	printf("going to sort gel\n"); */
/* 	fz_sort_gel(dev->gel); */

/* 	printf("\t left sort gel\n"); */
/* 	printf("goint to intersect\n"); */
...
/* { */
/* 	fz_draw_device *dev = devp->user; */
/* 	printf("matrix expansion in fz_ draw_stroke_path\n"); */
...
/* 	printf("heading to convert color\n"); */
/* 	fz_convert_color(dev->ctx, model, colorfv, colorspace, color); */
/* 	for (i = 0; i < model->n; i++) */
/* 		colorbv[i] = colorfv[i] * 255; */
/* 	colorbv[i] = alpha * 255; */

/* 	printf("heading to scan convert\n"); */
...
/* { */
/* 	fz_draw_device *dev = devp->user; */
/* 	printf("matrix expansion in fz_ draw_clip_path\n"); */
...
/* 	printf("matrix expansion in fz_ draw_clip_storke_path\n"); */
/* 	float expansion = fz_matrix_expansion(ctm); */
/* 	float flatness = 0.3f / expansion; */
/* 	float linewidth = stroke->linewidth; */
/* 	fz_irect bbox; */
...
/* 			int x = myfloorf(trm.e); */
/* 			int y = myfloorf(trm.f); */
...
fz_draw_fill_image(fz_device *devp, fz_image *image, const fz_matrix *ctm, float alpha)
{
	...
	dx = mysqrtf(local_ctm.a * local_ctm.a + local_ctm.b * local_ctm.b);
	dy = mysqrtf(local_ctm.c * local_ctm.c + local_ctm.d * local_ctm.d);
	...
	fz_try(ctx)
	{
		...
		/* Bevara- no pixmap transform allowed */
...
/* 	if (y0 > 0) */
/* 		y0 = 0; */
/* 	x0 = myfloorf((area.x0 + x0) / xstep); */
/* 	y0 = myfloorf((area.y0 + y0) / ystep); */
/* 	x1 = myceilf(area.x1 / xstep); */
/* 	y1 = myceilf(area.y1 / ystep); */
...
fz_new_draw_device(fz_context *ctx, fz_pixmap *dest)
{
	...
	fz_try(ctx)
	{
	  ddev->gel = NULL;  /* fz_new_gel(ctx); */
		ddev->flags = 0;
		ddev->ctx = ctx;
		ddev->top = 0;
		ddev->cache_x = NULL;  /* fz_new_scale_cache(ctx); */
		ddev->cache_y = NULL; /* fz_new_scale_cache(ctx); */
