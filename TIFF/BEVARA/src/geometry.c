...
fz_matrix_expansion(const fz_matrix *m)
{
 ...
 return mysqrtf((float) (fabsf(m->a * m->d - m->b * m->c)));
 /* return sqrtf((float) (fabsf(m->a * m->d - m->b * m->c))); */
...
{
	b->x0 = fz_clamp(myfloorf(r->x0), MIN_SAFE_INT, MAX_SAFE_INT);
	b->y0 = fz_clamp(myfloorf(r->y0), MIN_SAFE_INT, MAX_SAFE_INT);
	b->x1 = fz_clamp(myceilf(r->x1), MIN_SAFE_INT, MAX_SAFE_INT);
	b->y1 = fz_clamp(myceilf(r->y1), MIN_SAFE_INT, MAX_SAFE_INT);
	return b;
}
