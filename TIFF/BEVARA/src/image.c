...
fz_image_get_pixmap(fz_context *ctx, fz_image *image, int w, int h)
{
	...
	if (w == 0 || h == 0)
	...
	else
	  {
		...
		printf("expected size after loop = %d x %d\n",image->w, image->h);
	printf("12factor=%d\n",l2factor);
	...
	printf("looked for factor and tile\n");
	...
	case FZ_IMAGE_PNG:
	  ...
	  printf("NOT CURRENTLY HANDLING PNG\n");
	  exit(0);
		break;
	case FZ_IMAGE_TIFF:
	...
	case FZ_IMAGE_JXR:
	  ...
	  printf("JPEG-2000 currently disabled.\n");
	  exit(0);
		break;
	case FZ_IMAGE_JPEG:
		...
		/* 	} */
		/* } */
	  printf("Not handling JPEG external from  TIFF\n");
		/* fall through */
...
fz_new_image_from_buffer(fz_context *ctx, fz_buffer *buffer)
{
	...
	fz_try(ctx)
	{
		...
		{
		   printf("PNG imgaes currently disabled.\n");
		   exit(0);
