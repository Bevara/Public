...
/* a new fn to open the bevara inbuf */
fz_stream *fz_open_inbuf(fz_context *ctx, char *inbuf, int insize);


/* BEVARA: removed progressive handling, dont need it */
...
struct fz_stream_s
{
	...
	int error;
        ...
        int avail; /* available bytes */
        int INPUT_SIZE; /* really a repeat of avail, but implemented as backup */
        /* these two are also overkill, because info contained elsewhere */
        /* such as pos and INPUT_SIZE, but added in for ease of debugging */
        int INBUF_BYTES_START; 
        int INBUF_BYTES_LEFT; 
        int bits;  /* set of bits from stream */
        unsigned char *rp, *wp; /* read pointer and write pointer */
        void *state; /* contains fd and buffer */
        fz_stream_next_fn *next; /* fn fills up buffer*/
...
static inline int fz_read_byte(fz_stream *stm)
{
	...
	/* printf("fz_read_byte: reading a byte from the stream\n"); */


	if (stm->rp != stm->wp)
	  {
	    /* printf("\tincrementing the read ptr\n"); */
		return *stm->rp++;
	  }
	/* for debugging */
	/* else printf("\t\t skipping read ptr incr\n"); */


	fz_try(stm->ctx)
	{
	  /* printf("\t heading to next\n"); */
	  /* printf("\t start = %d, end =%d\n",stm->INBUF_BYTES_START,stm->INBUF_BYTES_LEFT); */

		c = stm->next(stm, 1);
		/* printf("\t\t got c=%c from next fn\n",(char)c); */
	}
	fz_catch(stm->ctx)
	{
	  /* printf("caught an error in fz_read_byte\n"); */
		fz_rethrow_if(stm->ctx, FZ_ERROR_TRYLATER);
		fz_warn(stm->ctx, "read error; treating as end of file");
		stm->error = 1;
		c = EOF;
