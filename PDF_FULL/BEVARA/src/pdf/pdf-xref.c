...
pdf_xref_entry *pdf_get_populating_xref_entry(pdf_document *doc, int num)
{
	...
	printf("in get populating xref entry...reading individ xref, num xref sections = %d\n",doc->num_xref_sections); 
	...
	{
	   printf("\t xref section loop, handling xref = %d\n",i); 
	...
	printf("retunring the populatin gsection \n");
...
pdf_read_new_xref_section(pdf_document *doc, fz_stream *stm, int i0, int i1, int w0, int w1, int w2)
{
	...
	printf("in read new xref section i0=%d, i1=%d, w0=%d, w1=%d, w2=%d\n",i0,i1,w0,w1,w2); 
	...
	for (i = i0; i < i0 + i1; i++)
	{
	  printf("\t getting new xref entry\n"); 
		pdf_xref_entry *entry = pdf_get_populating_xref_entry(doc, i);
		int a = 0;
		int b = 0;
		int c = 0;

		printf("back from get populating\n"); fflush(stdout);
		if (fz_is_eof(stm))
			fz_throw(doc->ctx, FZ_ERROR_GENERIC, "truncated xref stream");

		
		printf("checked EOF going to read some bytes\n"); fflush(stdout);
		for (n = 0; n < w0; n++)
		...
		printf("done with reading the bytes\n"); fflush(stdout);
		if (!entry->type)
		{
			int t = w0 ? a : 1;
	...
	printf("setting has xref streams\n"); fflush(stdout);
...
pdf_read_new_xref(pdf_document *doc, pdf_lexbuf *buf)
{
	...
	fz_try(ctx)
	{
	  ...
	  printf("\t getting size from trailer\n"); 
 ...
 printf("\t getting W from trailer\n"); 
		...
		printf("\t getting Index from trailer\n"); 
		index = pdf_dict_gets(trailer, "Index");


printf("opening stream wiht offset\n");
		...
		else
		{
		   printf("\t\t index found \n"); 
			...
			for (t = 0; t < n; t += 2)
			{
				...
				printf("going to read new xref section\n"); fflush(stdout);
...
pdf_read_xref(pdf_document *doc, int ofs, pdf_lexbuf *buf)
{
	...
	printf("reading the xref...could put in  new code\n"); 
...
read_xref_section(pdf_document *doc, int ofs, pdf_lexbuf *buf, ofs_list *offsets)
{
	...
	fz_var(trailer);
	 ...
	 printf("\t reading an xref section\n"); 
	...
	fz_try(ctx)
	{
		...
		if (xrefstmofs)
		{
			...
			/*
				...
				1.7
			*/
			printf("\t found XREF dropping obj\n"); 
...
pdf_read_xref_sections(pdf_document *doc, int ofs, pdf_lexbuf *buf, int read_previous)
{
	...
	fz_try(ctx)
	{
		while(ofs)
		{
			printf("\t\t going to pop next xref level\n");
			pdf_populate_next_xref_level(doc);
			printf("\t\t goint to read xref section \n");
...
pdf_load_xref(pdf_document *doc, pdf_lexbuf *buf)
{
	...
	printf("\tread start xref\n");  fflush(stdout);

	pdf_read_start_xref(doc);

	printf("\t read xref sections\n"); 
...
pdf_init_document(pdf_document *doc)
{
	...
	/* printf("set up progressive vs linear reading\n"); */
	...
	{
		char *version_str;
		/* printf("Heading to grab Root/Version\n"); */
...
pdf_meta(pdf_document *doc, int key, void *ptr, int size)
{
	...
	*/
	...
	case FZ_META_HAS_PERMISSION:
	{
		...
		/* inserted pdf_has_permission fn here */
		if (!doc->crypt)
		  return 1;
		printf("encrypted -- EXITING\n"); exit(0);/*doc->crypt->i & i;*/
		/* end of insert */
...
pdf_new_document(fz_context *ctx, fz_stream *file)
{
	...
	doc->super.needs_password = 0; /*(fz_document_needs_password_fn *)pdf_needs_password; */ /* assume password already checked in Preserve */
	doc->super.authenticate_password = NULL; /* (fz_document_authenticate_password_fn *)pdf_authenticate_password; */ /* removed password authentication */
	doc->super.load_outline = NULL; /*(fz_document_load_outline_fn *)pdf_load_outline; */ /*assume we don't need outline */
...
pdf_open_document_no_run(fz_context *ctx, const char *filename)
{
	...
	/* 	printf("tyring for new document\n"); */
	/* 	doc = pdf_new_document(ctx, file); */
	/* 	printf("trying for doc init\n"); */
	/* 	pdf_init_document(doc); */
	/* 	printf("opened the doc\n"); */
	/* } */
	/* fz_always(ctx) */
	/* { */
	/* 	fz_close(file); */
