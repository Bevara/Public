...
//    modified code.
////////////////////////////////////////////////////////////////////////////////
// This software modified by BEVARA 
//   and inherits the KeyJ Research License
//
// 2014: Bevara has modified the code to include an entry point, a setup function
// 		and a shutdown function. We have modified the code to support RGBA output;
//		greyscale output is removed.
// 2015: Bevara removed setup. Init and entry functions remain for testing.  
// 2017: Bevara has modified the code to support EMSCRIPTEN  
...
// BEVARA: use the following for test purposes
//
...
// int main(int argc, char* argv[]) {
	...
	// njDecode(inbuf, insize);
    // outwidth = njGetWidth();
	// outheight = njGetHeight();
	// outbuf = njGetImage();


    // f2 = fopen("nanojpeg_out.ppm", "wb");
    // if (!f2) {
        // printf("Error opening the output file.\n");
        // return 1;
    // }
    // fprintf(f2, "P6\n%d %d\n255\n", outwidth, outheight);
    // fwrite(outbuf, 1, outwidth*outheight*3, f2);
    // fclose(f2);

    //njDone();
	// int i;
	// for (i=0; i< outwidth*outheight*3; ++i)
...
typedef struct
{
	...
	short ES_ID;
	int BPP, width, height, out_size, pixel_format;
} JPEGDec;

int nanojpeg_init(void* data, unsigned int dataLength){
	njInit();
	return 0;
}

int nanojpeg_shutdown(void)
{
	/* clean up */
	 njDone();
	return 0;
...
{
    if (njDecode(inBuffer, inBufferLength)) {
        return -5;
    }

	if (njGetImageSize() > *outBufferLength){
		*outBufferLength = njGetImageSize();
		return -9;
	}
...
NJ_INLINE void njDecodeSOF(void) {
    ...
    // BEVARA: we'll output RGBA even if grayscale, so go ahead and
    // hardcode the malloc
    //if (nj.ncomp == 3) {
    nj.rgb = (unsigned char*)malloc(nj.width * nj.height * 4); // nj.ncomp);
...
NJ_INLINE void njConvert() {
    ...
    if (nj.ncomp == 3) {
        ...
        // BEVARA: we're outputting RBG for grayscale, so re-write 
        // the following
	...
	nj.ncomp = 3; // make sure we let everyone know we're now RGB
...
int EMSCRIPTEN_KEEPALIVE njGetWidth(void)            { printf("in get width val is %d\n", nj.width); return nj.width; }
int EMSCRIPTEN_KEEPALIVE njGetHeight(void)           { printf("in get height val is %d\n", nj.height); return nj.height; }
int njIsColor(void)             { return (nj.ncomp != 1); }
unsigned char* EMSCRIPTEN_KEEPALIVE njGetImage(void) 
	{ 
